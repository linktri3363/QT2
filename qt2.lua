...
autoConfirmReclaimer = true -- Auto-confirm A.M.A.N. Reclaimer trades
    ...
    function(...)
        ...
        elseif arg[1] and arg[1]:lower() == "xyz" then
            ...
            if target and zone then
                print(
                    string.format(
                        _addon.name ..
                            ": Detected coordinates for " .. target.name .. ': Zone: "%s", X: %s, Y: %s, Z: %s',
                        zone,
                ...
                print(
                    _addon.name ..
                        ": Example mode enabled. " ..
                            _addon.name ..
                                " will not trade items. It will only show the command to be used in the console."
                )
            else
        ...
        elseif arg[1] and arg[1]:lower() == "fake" and arg[2] and arg[2]:lower() ~= "" then
            ...
            if fakeNPC ~= "" then
                print(
                    _addon.name ..
                        ": Now assuming the player is targetting and trading with the NPC " ..
                            fakeNPC .. ". For testing purposes only, trading disabled."
                )
                exampleOnly = true
            else
            ...
            return
        elseif arg[1] and arg[1]:lower() == "autoconfirm" then
            autoConfirmReclaimer = not autoConfirmReclaimer
            if autoConfirmReclaimer then
                print(_addon.name .. ": Auto-confirm for A.M.A.N. Reclaimer ENABLED")
            else
                print(_addon.name .. ": Auto-confirm for A.M.A.N. Reclaimer DISABLED")
            end
            return
        end
...
function playerMoved()
    playerCurrentPosition = {
        [1] = windower.ffxi.get_mob_by_target("me").x,
        [2] = windower.ffxi.get_mob_by_target("me").y
    }

    if
        #playerLastPosition ~= 0 and
            (playerLastPosition[1] ~= playerCurrentPosition[1] or playerLastPosition[2] ~= playerCurrentPosition[2])
     then
        if debug then
            print(_addon.name .. ": Movement detected. Cancelling.")
        end
        playerCurrentPosition = {}
        playerLastPosition = {}
        reset()
        ...
        for i = 1, #tradeGroup[k] do -- Iterate through each item in the "standalone" table
            ...
            if not duplicateFound then
                for key in pairs(res.items) do
                    ...
                    if res.items[key] then
                        ...
                        if
                            tradeGroup[k][i].item and
                                (res.items[key].en:lower() == tradeGroup[k][i].item:lower() or
                                    res.items[key].enl:lower() == tradeGroup[k][i].item:lower())
                         then -- Check that the item matches one of the two names in items.lua
            ...
            if not tradeGroup[k][i].item or not matchFound then
                ...
                table.insert(
                    ...
                    #foundItemsTable + 1,
                    {
                        ["item"] = tradeGroup[k][i].item,
                        ["id"] = tradeGroup[k][i].id,
                        ["maxStackSize"] = tradeGroup[k][i].maxStackSize,
                        ["fullName"] = tradeGroup[k][i].fullName
                    }
                )
            end
    ...
    if #unmatchedItems > 0 then
        ...
        print(
            _addon.name ..
                ": " ..
                    _addon.name ..
                        " disabled. No matches for the following items. Edit or remove these entries and reload the addon. : " ..
                            unmatchedItemsString
        )
        itemVerificationComplete = false
    else
        print(
                        ...
                        " / " ..
                            userSpecifiedItems ..
                                " matches, " ..
                                    partialMatches ..
                                        " partial " ..
                                            (partialMatches == 1 and "match" or "matches") ..
                                                " also accepted. Quicktrade is ready."
        )
        itemVerificationComplete = true
        if debug then
            print("resourceItemsChecked Total", resourceItemsCheckedTotal)
        end
        if debug then
            print("Time to verify items:", math.round(os.clock() - startTime, 3) .. "s")
        end
    end
end
...
function verifyNPCAndGetTrades() -- Check the currently target, or target the nearest NPC, verify they have trades configures, define the itemType, and build the itemTable
    ...
    for tradeType, tradeNPCs in pairs(tradeGroup) do
        for i = 1, #tradeGroup[tradeType].NPC do
            if target.name == tradeGroup[tradeType].NPC[i].name then
                if debug then
                    print("Target is either a match, or fakeNPC is set")
                end

                if tradeGroup[tradeType].NPC[i].zone then
                    if debug then
                        print("Zone Specified:", tradeGroup[tradeType].NPC[i].zone, "Player Zone:", zone)
                    end

                    if
                        tradeGroup[tradeType].NPC[i].x and tradeGroup[tradeType].NPC[i].y and
                            tradeGroup[tradeType].NPC[i].z and
                            tradeGroup[tradeType].NPC[i].zone == zone
                     then
                        if debug then
                            print("Checking precise NPC coordinates")
                        end
                        if fakeNPC ~= "" then
                            distanceToCoordinates = 1
                        else
                            distanceToCoordinates =
                                ((tradeGroup[tradeType].NPC[i].x - player.x) ^ 2 +
                                (tradeGroup[tradeType].NPC[i].y - player.y) ^ 2 +
                                (tradeGroup[tradeType].NPC[i].z - player.z) ^ 2):sqrt()
                        end

                        if debug then
                            print("distance", math.round(distanceToCoordinates, 2))
                        end
    ...
    if debug then
        print("verifyNPCAndGetTrades() results:", target.name, "Trade: " .. itemType)
    end
    ...
    if itemType == "" then
        ...
        if fakeNPC == "" and target.distance:sqrt() > maxTradeDistance then -- 5.5 Yalms, according to DistancePlus
            if debug then
                print(_addon.name .. ": " .. target.name .. " is too far away")
            end
...
function calculateAndTrade() -- Scan inventory, calculate possible trades, and build the TradeNPC command
        ...
        if bagItems then
            for itemNumber = 1, #tradeGroup[itemType] do
                if tempTableInventory[itemNumber].group and #groups == 0 then -- Checks tag: group    Only runs if the groups table hasn't been built
                    if debug then
                        print("Adding First Group", tempTableInventory[itemNumber].group)
                    end
                    ...
                    for x = 1, #tempTableInventory do -- When the first item is found that has a group specified, scan items to determine other groups and place them in the table
                        if tempTableInventory[x].group then
                            ...
                            if not groupPreviouslyFound then
                                if debug then
                                    print("Adding Group", tempTableInventory[x].group)
                                end
                                table.insert(
                                    groups,
                                    #groups + 1,
                                    {["name"] = tempTableInventory[x].group, ["complete"] = false}
                                ) -- Complete is check later, and is set to 'true' if all item requirements are met.
                            end
                        end
                    end
                ...
                for bagItem = 1, #bagItems do
                    if
                        bags[b] == "inventory" and tempTableInventory[itemNumber].partialMatch and
                            not tradeGroup[itemType][itemNumber].matched and
                            tradeGroup[itemType][itemNumber].id == 0 and
                            bagItems[bagItem].id > 0 and
                            string.find(
                                res.items[bagItems[bagItem].id].enl:lower(),
                                tempTableInventory[itemNumber].item:lower()
                            )
                     then
                        print(
                            _addon.name .. ": " .. tempTableInventory[itemNumber].item,
                            "is possibly:",
                            res.items[bagItems[bagItem].id].enl:lower()
                        )
                    ...
                    if bagItems[bagItem].id > 0 and bagItems[bagItem].id == tradeGroup[itemType][itemNumber].id then
                        if bags[b] == "inventory" then
                            ...
                            if tempTableInventory[itemNumber].keyItem ~= nil then -- Skip the item if the key item is specified and the player already has it
                                ...
                                if not keyItemNeeded then
                                    if debug then
                                        print(
                                            "Key item " ..
                                                tempTableInventory[itemNumber].keyItem ..
                                                    " already in possession. Skipping item"
                                        )
                                    end
                                    break
                                end
                            ...
                            if
                                tempTableInventory[itemNumber].zone and
                                    res.zones[windower.ffxi.get_info().zone].name ~= tempTableInventory[itemNumber].zone
                             then
                                if debug then
                                    print(
                                        "Excluding :" ..
                                            tempTableInventory[itemNumber].item ..
                                                ". It is intended for zone: " .. tempTableInventory[itemNumber].zone
                                    )
                                end
                                break
                            end

                            tempTableInventory[itemNumber].count =
                                tempTableInventory[itemNumber].count + bagItems[bagItem].count
                            inventoryItems = inventoryItems + bagItems[bagItem].count
                            if debug then
                                print(
                                    tempTableInventory[itemNumber].item .. " in Inventory:",
                                    bagItems[bagItem].count,
                                    "Total Inventory Count:",
                                    inventoryItems
                                )
                            end
                        elseif T({"safe", "safe2", "storage", "locker"}):contains(bags[b]) then
                            tempTableMoogleOnly[itemNumber].count =
                                tempTableMoogleOnly[itemNumber].count + bagItems[bagItem].count
                            inaccessibleItems = inaccessibleItems + bagItems[bagItem].count
                            if debug then
                                print(
                                    tempTableMoogleOnly[itemNumber].item .. " in Mog House:",
                                    bagItems[bagItem].count,
                                    "Total Mog House Count:",
                                    inaccessibleItems
                                )
                            end
                        else
                            tempTableAnywhere[itemNumber].count =
                                tempTableAnywhere[itemNumber].count + bagItems[bagItem].count
                            availableItems = availableItems + bagItems[bagItem].count
                            if debug then
                                print(
                                    tempTableAnywhere[itemNumber].item .. " in Available Storage:",
                                    bagItems[bagItem].count,
    ...
    for i = 1, #groups do -- Go through any groups and see if one is complete. Will trade only the items specified in the first complete group
        if debug then
            print("Scanning Group:", groups[i].name)
        end
        groupReady = true
        keyItemOwned = false
        ...
        for o = 1, #tempTableInventory do
            if tempTableInventory[o].group and tempTableInventory[o].group == groups[i].name then
                if debug then
                    print("Checking Group :", tempTableInventory[o].group)
                end

                if not tempTableInventory[o].tradeReady then
                    if debug then
                        print("Trade Not Ready:", tempTableInventory[o].item)
                    end
                    groupReady = false
                    break
                end
        ...
        if groupReady then
            for o = 1, #ownedGeasFeteKeyItems do
                if ownedGeasFeteKeyItems[o].name == groups[i].keyItem then
                    if debug then
                        print("Key Item: " .. groups[i].keyItem .. " already in possession. Ignoring trade.")
                    end
                    keyItemOwned = true
                end
            end
            ...
            if not keyItemOwned then
                if debug then
                    print("Group: " .. groups[i].name .. " is ready to trade")
                end
                ...
                for o = 1, #tempTableInventory do
                    if tempTableInventory[o].group == groups[i].name then
                        tradeCommand =
                            tradeCommand ..
                            " " .. tempTableInventory[o].exact .. ' "' .. tempTableInventory[o].item .. '"'
        ...
        while (loops.requested or (not loops.requested and curLoop == 0)) and curLoop ~= maxLoops do
            ...
            for i = 1, #tempTableInventory do
                if debug then
                    print(
                        i,
                        tempTableInventory[i].count,
                        tempTableAnywhere[i].count,
                        tempTableMoogleOnly[i].count,
                        tempTableInventory[i].item
                    )
                end
                ...
                if pullEnabled and tempTableAnywhere[i].count > 0 and (loops.requested and not loops.allowed) then
                    if debug then
                        print(
                            "Merging Inventory and Anywhere tables to calculate loop count:",
                            tempTableAnywhere[i].item,
                            tempTableInventory[i].count,
                            "+",
                            tempTableAnywhere[i].count
                        )
                ...
                if
                    pullEnabled and tempTableAnywhere[i].count > 0 and
                        ((loops.requested and not loops.allowed) or (not loops.requested and not loops.allowed))
                 then
                    ...
                    if not exampleOnly and (inventorySlotsOpen > 0 or exampleOnly) then
                        ...
                        while inventorySlotsOpen > 0 and tempTableAnywhere[i].count > 0 do
                            pullCount =
                                pullCount + math.min(tempTableAnywhere[i].count, tempTableAnywhere[i].maxStackSize)
                            tempTableAnywhere[i].count =
                                tempTableAnywhere[i].count -
                                math.min(tempTableAnywhere[i].count, tempTableAnywhere[i].maxStackSize)
                        ...
                        if debug then
                            print(
                                "Pulling: " ..
                                    "get " ..
                                        tempTableAnywhere[i].item ..
                                            " " .. pullCount .. " + " .. tempTableInventory[i].count
                            )
                        end
                        if debug then
                            print("Expected Inventory Slots Open:", inventorySlotsOpen)
                        end

                        if not exampleOnly then
                            windower.send_command(
                                "get " .. tempTableAnywhere[i].item .. " " .. (pullCount + tempTableInventory[i].count)
                            )
                            itemsPulled = true
                ...
                if tradeSlotsAvailable > 0 and tempTableInventory[i].count > 0 then
                    if tempTableInventory[i].exact and tempTableInventory[i].count < tempTableInventory[i].exact then -- Go to next item if there aren't enough to meet the 'exact' requirement
                        if debug then
                            print("Not enough for exact trade", tempTableInventory[i].item, tempTableInventory[i].count)
                        end
                        break
                    end
                    ...
                    while tradeSlotsAvailable > 0 and currentItemsAccountedFor < tempTableInventory[i].count do
                        if debug then
                            print("maxStackSize", tempTableInventory[i].maxStackSize)
                        end
                        currentItemsAccountedFor =
                            currentItemsAccountedFor +
                            math.min(currentItemOriginalTotal, tempTableInventory[i].maxStackSize)
                        ...
                        if loops.requested then
                            if debug then
                                print(
                                    ...
                                    tempTableInventory[i].count,
                                    "to",
                                    math.max(0, tempTableInventory[i].count - currentItemsAccountedFor)
                                )
                            end
                            tempTableInventory[i].count =
                                math.max(0, tempTableInventory[i].count - currentItemsAccountedFor)
                            ...
                            if tempTableInventory[i].unique then
                                if tradeCommand ~= "tradenpc" then
                                    loops.calculated = loops.calculated + 1
                                end
                                tradeCommand =
                                    tradeCommand ..
                                    " " .. currentItemsAccountedFor .. ' "' .. tempTableInventory[i].item .. '"'
                                loops.uniqueTrades = loops.uniqueTrades + 1
                                uniqueTradeAccountedFor = true
                                if debug then
                                    print("Unique trades:", loops.uniqueTrades)
                                end
                                break
                            else
                                tradeCommand =
                            ...
                            if not commandComplete then
                                if tradeCommand ~= "tradenpc" and tempTableInventory[i].unique then
                                    if debug then
                                        print("Skipping unique item when a command already contains non-unique items")
                                    end
                                    break
                                else
                                    tradeCommand =
            ...
            if tradeSlotsAvailable == 8 then
                ...
                if debug and loops.requested then
                    print("final loops.calculated", loops.calculated)
                end
                break
            elseif loops.requested then
                if not uniqueTradeAccountedFor then
                    loops.calculated = loops.calculated + 1
                end

                if debug then
                    print("loops.calculated", loops.calculated)
                end
                if debug then
                    print("Expected Command:", tradeCommand)
                end
            end
    ...
    if
        inventoryItems == 0 and (not loops.requested or (loops.allowed and loops.calculated == 0)) and
            (not pullEnabled or (pullEnabled and not itemsPulled))
     then
        ...
        if availableItems > 0 then
            windower.add_to_chat(
                ...
                _addon.name ..
                    ": " ..
                        availableItems ..
                            ((availableItems == 1) and " item" or " items") ..
                                " in your satchel/sack/case can be traded to " ..
                                    currentTradeTargetName .. '. Use "qt2 pull" to pull and trade them'
            )
        end
    ...
    for i = 1, #tradeGroup[itemType] do
        if tradeGroup[itemType][i].matched then
            if debug then
                print(
                    "Resetting " ..
                        tradeGroup[itemType][i].item .. " to user info: " .. tradeGroup[itemType][i].itemByUser
                )
            end
            tradeGroup[itemType][i] = {
                ["item"] = tradeGroup[itemType][i].itemByUser,
                ["partialMatch"] = true,
                ["matched"] = false
            }
        end
    ...
    if itemsPulled then
        if debug then
            print("Waiting 1.5 seconds for pulled items")
        end
        windower.add_to_chat(
            chatColor,
            _addon.name ..
                ": Pulling " .. pullCountTotal .. ((pullCountTotal == 1) and " item" or " items") .. " from storage"
        )
        coroutine.sleep(1.5)
    end
    ...
    if tradeCommand ~= "tradenpc" and (not loops.requested or loops.allowed) then
                            ...
                            loops.current + 1 ..
                                "/" ..
                                    loops.calculated ..
                                        ", " ..
                                            totalItemsInTrade ..
                                                (totalItemsInTrade == 1 and " item" or " items") ..
                                                    " --> " .. currentTradeTargetName
                )
            else
                windower.add_to_chat(
                    ...
                    _addon.name ..
                        ": " ..
                            totalItemsInTrade ..
                                (totalItemsInTrade == 1 and " item" or " items") .. " --> " .. currentTradeTargetName
                )
            end
        ...
        if debug then
            print("Trade Slots Remaining:", tradeSlotsAvailable)
        end
    end

    if loops.requested and loops.calculationComplete then
        if debug then
            print("Loops now allowed")
        end
        loops.allowed = true
        loops.requested = false
    end
...
function getGeasFeteKeyItems()
    if debug then
        print("Scanning Geas Fete Key Items")
    end
...
windower.register_event(
    "outgoing chunk",
    function(packetID, original, modified, injected)
        ...
        elseif status_TradeInProgress and old == 4 and new == 0 then
            ...
            if loops.allowed then
                ...
                if loops.current < loops.calculated then
                    ...
                    if debug then
                        print("Loop Completed: " .. loops.current .. "/" .. loops.calculated)
                    end
                    verifyNPCAndGetTrades:schedule(0.5)
                else
                    if debug then
                        print("Total Loops Completed: " .. loops.current)
                    end
                    reset()
                end
            end
...
function targetQuestionMark()
    ...
    for _, mob in pairs(mobs) do
        if mob.valid_target and mob.name == "???" then
            if debug and mob.name == "???" then
                print("??? Distance", mob.distance:sqrt())
            end
...
function tripleQuestionmarkSpawnNotification(id, data)
    if
        not tripleQuestionmarkAlert or os.clock() - tripleQuestionmarkNotificationTime < 20 or
            not res.zones[windower.ffxi.get_info().zone].name:startswith("Abyssea")
     then
        return
    end
            ...
            for tradeType, tradeNPCs in pairs(tradeGroup) do
                for i = 1, #tradeGroup[tradeType].NPC do
                    if tradeGroup[tradeType].NPC[i].name == "???" then
                        if
                            tradeGroup[tradeType].NPC[i].zone and
                                tradeGroup[tradeType].NPC[i].zone == res.zones[windower.ffxi.get_info().zone].name
                         then
                            if
                                tradeGroup[tradeType].NPC[i].x and tradeGroup[tradeType].NPC[i].y and
                                    tradeGroup[tradeType].NPC[i].z and
                                    (string.format("%.2f", tradeGroup[tradeType].NPC[i].x) ==
                                        string.format("%.2f", npc.x) and
                                        string.format("%.2f", tradeGroup[tradeType].NPC[i].y) ==
                                            string.format("%.2f", npc.y) and
                                        string.format("%.2f", tradeGroup[tradeType].NPC[i].z) ==
                                            string.format("%.2f", npc.z))
                             then
                                distanceToSpawn =
                                    ((tradeGroup[tradeType].NPC[i].x - _player.x) ^ 2 +
                                    (tradeGroup[tradeType].NPC[i].y - _player.y) ^ 2 +
                                    (tradeGroup[tradeType].NPC[i].z - _player.z) ^ 2):sqrt()
...
-- Auto-confirm A.M.A.N. Reclaimer trades
windower.register_event(
    "incoming text",
    function(original, modified, mode)
        if not autoConfirmReclaimer then
            return
        end
        
        if debug and original:find("A.M.A.N. Reclaimer") then
            print("Reclaimer text detected: " .. original:sub(1, 50))
        end
        
        -- Detect the Reclaimer confirmation prompt (Would you like 600 reclamation marks)
        if original:find("Would you like") and original:find("reclamation") then
            if debug then
                print("Confirmation prompt detected!")
            end
            coroutine.schedule(function()
                windower.send_command("setkey up down; wait 0.1; setkey up up; wait 0.1; setkey enter down; wait 0.1; setkey enter up")
                if debug then
                    print("Auto-confirmed A.M.A.N. Reclaimer trade")
                end
                -- Re-target after confirming
                coroutine.schedule(function()
                    windower.send_command("input /targetnpc")
                end, 0.5)
            end, 0.3)
...
tradeGroup.SalvagePlan = {
    -- Status: Untested. Do these have to be done separately?
    NPC = {
        {name = "Mrohk Sahjuuli"}
    },
...
tradeGroup.JSECape = {
    ...
    {item = "Mauler's Mantle", unique = true, max = 1},
    {item = "Anchoret's Mantle", unique = true, max = 1},
    {item = "Mending Cape", unique = true, max = 1},
    {item = "Bane Cape", unique = true, max = 1},
    {item = "Ghostfyre Cape", unique = true, max = 1},
    {item = "Canny Cape", unique = true, max = 1},
    {item = "Weard Mantle", unique = true, max = 1},
    {item = "Niht Mantle", unique = true, max = 1},
    {item = "Pastoralist's Mantle", unique = true, max = 1},
    {item = "Rhapsode's Cape", unique = true, max = 1},
    {item = "Lutian Cape", unique = true, max = 1},
    {item = "Takaha Mantle", unique = true, max = 1},
    {item = "Yokaze Mantle", unique = true, max = 1},
    {item = "Updraft Mantle", unique = true, max = 1},
    {item = "Conveyance Cape", unique = true, max = 1},
    {item = "Cornflower Cape", unique = true, max = 1},
    {item = "Gunslinger's Cape", unique = true, max = 1},
    {item = "Dispersal Mantle", unique = true, max = 1},
    {item = "Toetapper Mantle", unique = true, max = 1},
    {item = "Bookworm's Cape", unique = true, max = 1},
    {item = "Lifestream Cape", unique = true, max = 1},
    {item = "Evasionist's Cape", unique = true, max = 1},
    {item = "Mecistopins Mantle", unique = true, max = 1}
}

tradeGroup.AbysseaChests = {
    -- Status: Testing complete. Opens all chests
    NPC = {
        {name = "Sturdy Pyxis"}
    },
...
tradeGroup.TreasureCoffer = {
    -- Status: Tested in Oztroja
    NPC = {
...
tradeGroup.MogHouseMoogle = {
    -- Status: Tested in Bastok and Whitegate
    NPC = {
...
tradeGroup.FestiveMoogle = {
    -- Status: Tested Red in Bastok
    NPC = {
        {name = "Festive Moogle"}
    },
...
tradeGroup.PostmanAlwaysKOsTwice = {
    -- Status: Untested
    NPC = {
        {name = "Ambrosius"}
    },
...
tradeGroup.ItSetsMyHeartAflutter = {
    -- Status: Untested
    NPC = {
        {name = "Saldinor"}
    },
...
tradeGroup.LoggingPoint = {
    -- Status: Testing Complete, cannot loop
    NPC = {
        {name = "Logging Point"}
    },
...
tradeGroup.AbysseaAttohwaPallidPercy = {
    -- Status: Confirmed
    NPC = {
...
tradeGroup.AbysseaAttohwaWherwetrice = {
    -- Status: Confirmed
    NPC = {
...
tradeGroup.AbysseaGraubergFleshflayerKillakriq = {
    -- Status: Confirmed
    NPC = {
...
tradeGroup.AbysseaGraubergBomblixFlamefinger = {
    -- Status: Confirmed
    NPC = {
...
tradeGroup.AbysseaAltepaChickcharney = {
    -- Status: Confirmed
    NPC = {
...
tradeGroup.AbysseaMisareauxNehebkau = {
    -- Status: Confirmed
    NPC = {
